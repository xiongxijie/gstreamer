...
printf ("(gst_pad_init) GST_PAD_SET_FLUSHING \n");
...
no_parent:
  {
        printf ("(gst_pad_set_active) no parent \n");
...
failed:
  {
    ...
    if (!active) {
      g_critical ("Failed to deactivate pad %s:%s, very bad",
        ...
        printf ("(gst_pad_set_active) Failed to activate pad \n");
  ...
  if (data->event && GST_EVENT_IS_STICKY (data->event) &&
    ...
    data->ret = gst_pad_push_event_unchecked (pad, gst_event_ref (event),
        GST_PAD_PROBE_TYPE_EVENT_DOWNSTREAM);
          ...
          // int xx = (int)GST_PAD_IS_FLUSHING (pad);
          // printf ("(gst_pad/push_sticky) will call gst_pad_push_event_unchecked %d xx=%d\n", (int)data->ret,xx);
...
gst_pad_push_data (GstPad * pad, GstPadProbeType type, void *data)
{
  ...
  if (G_UNLIKELY (GST_PAD_IS_FLUSHING (pad)))
  {
 printf("(gst_pad_push_data) goto flushing \n");
    goto flushing;
  }

  if (G_UNLIKELY (GST_PAD_IS_EOS (pad)))
  {
printf("(gst_pad_push_data) goto eos \n");
    goto eos;
  }
...
flushing:
  {
      ...
      printf("(gst_pad_push_data) pushing, but pad was flushing\n");
...
eos:
  {
      ...
      printf("(gst_pad_push_data) pushing, but pad was EOS\n");
...
flushed:
  {
        ...
        printf ("(store_sticky_event) pad is flushing \n");
...
flushed:
  {
    ...
    printf ("(gst_pad_push_event) eventname:%s , we are flushing \n",GST_EVENT_TYPE_NAME (event));
...
eos:
  {
    printf ("(gst_pad_push_event) eventname:%s , we are eos \n",GST_EVENT_TYPE_NAME (event));
    ...
    printf ("(gst_pad_push_event) eventname:%s , we are done \n",GST_EVENT_TYPE_NAME (event));
...
flushing:
  {
        printf ("(Received event on flushing pad. Discarding) %s\n",
        GST_EVENT_TYPE_NAME (event));
    GST_OBJECT_UNLOCK (pad);
    if (need_unlock)
      GST_PAD_STREAM_UNLOCK (pad);
