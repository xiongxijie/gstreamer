    ...
    guint probability, GstCaps * caps)
{
          ...
          printf("(gstTYPEFIND) gst_type_find_element_emit_have_type \n");
...
#if 2
  typefind->have_type_emitted = TRUE;
#endif
...
#if 2
   g_object_class_install_property (gobject_class, PROP_HAVE_TYPE_EMITTED,
      g_param_spec_boolean ("have-type-emitted", "have-type signal emitted",
          "Tell btdemux that have-type signal emitted",
          FALSE,
          G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
#endif
    ...
    const GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    case PROP_FORCE_CAPS:
      ...
      break;
#if 2
    case PROP_HAVE_TYPE_EMITTED:
          printf ("(gst_type_find_element_set_property) \n");
      typefind->have_type_emitted = g_value_get_boolean (value);
      break;
#endif
    default:
...
#if 2
    case PROP_HAVE_TYPE_EMITTED:
      g_value_set_boolean (value, typefind->have_type_emitted);
      break;
#endif
...
start_typefinding (GstTypeFindElement * typefind)
{
      ...
      printf ("(gstTYPEFIND) start_typefinding \n");
