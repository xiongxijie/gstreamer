...
{
                              printf("(gst_download_buffer_init) \n");
...
update_levels (GstDownloadBuffer * dlbuf, guint bytes)
{
  ...
  GST_DEBUG ("levels: bytes %u/%u, time %" GST_TIME_FORMAT "/%" GST_TIME_FORMAT,
      ...
      GST_TIME_ARGS (dlbuf->max_level.time));
            ...
            printf("(gstdownloadbuffer/update_levels) levels: bytes %u/%u, time time %" GST_TIME_FORMAT "/%" GST_TIME_FORMAT" \n",
            dlbuf->cur_level.bytes, dlbuf->max_level.bytes,
            GST_TIME_ARGS (dlbuf->cur_level.time),
            GST_TIME_ARGS (dlbuf->max_level.time));
}
...
perform_seek_to_offset (GstDownloadBuffer * dlbuf, guint64 offset)
{
                      ...
                      printf("(downloadbuffer/perform_seek_to_offset) seeking to %ld \n", offset);
    ...
    guint length)
{
  ...
  gboolean started;
                  ...
                  printf("(gst_download_buffer_wait_for_data) wait for %" G_GUINT64_FORMAT ", length %u \n"
                  ,offset
                  ,length);
  ...
  if (gst_sparse_file_get_range_before (dlbuf->file, offset, &start, &stop)) {
    GST_DEBUG_OBJECT (dlbuf,
        "range before %" G_GSIZE_FORMAT " - %" G_GSIZE_FORMAT, start, stop);
                ...
                printf("(gst_download_buffer_wait_for_data) range before %" G_GSIZE_FORMAT " - %" G_GSIZE_FORMAT " \n",
                start,
                stop);
    ...
    guint length, GstBuffer ** buffer)
{
              ...
              printf("(gst_download_buffer_read_buffer) Reading %u bytes from %" G_GUINT64_FORMAT "\n",
              length, offset);
...
gst_download_buffer_open_temp_location_file (GstDownloadBuffer * dlbuf)
{
  ...
  GST_DEBUG_OBJECT (dlbuf, "opening temp file %s", dlbuf->temp_template);
            ...
            printf("(gst_download_buffer_open_temp_location_file)opening temp file %s\n",
            dlbuf->temp_template);
...
{
                  printf("(gst_download_buffer_flush_temp_file) \n");
  if (dlbuf->file == NULL)
    return;
    ...
    GstEvent * event)
{
  ...
  switch (GST_EVENT_TYPE (event)) {
    ...
    {
                  printf("(gst_download_buffer_handle_sink_event) GST_EVENT_FLUSH_START\n");
    ...
    case GST_EVENT_FLUSH_STOP:
    {
                  ...
                  printf("(gst_download_buffer_handle_sink_event) GST_EVENT_FLUSH_STOP\n");
    ...
    default:
      if (GST_EVENT_IS_SERIALIZED (event)) {
        ...
        switch (GST_EVENT_TYPE (event)) {
          case GST_EVENT_EOS:
                  printf("(gst_download_buffer_handle_sink_event) GST_EVENT_EOS\n");
          ...
          case GST_EVENT_SEGMENT:
                  printf("(gst_download_buffer_handle_sink_event) GST_EVENT_SEGMENT\n");
          ...
          case GST_EVENT_STREAM_START:
                  printf("(gst_download_buffer_handle_sink_event) GST_EVENT_STREAM_START\n");
...
gst_download_buffer_chain (GstPad * pad, GstObject * parent, GstBuffer * buffer)
{
  ...
  dlbuf = GST_DOWNLOAD_BUFFER (parent);
                  ...
                  printf("(gst_download_buffer_chain) receive buffer of size %" G_GSIZE_FORMAT " \n", 
  ...
  GST_DEBUG_OBJECT (dlbuf, "Writing %" G_GSIZE_FORMAT " bytes to %"
      G_GUINT64_FORMAT, info.size, offset);
              ...
              printf("(gst_download_buffer_chain) writing %" G_GSIZE_FORMAT " bytes to %" G_GUINT64_FORMAT " \n",
              info.size,
              offset);
  ...
  if (dlbuf->write_pos + available == dlbuf->upstream_size) {
    ...
    if (gst_sparse_file_get_range_after (dlbuf->file, 0, &start, &stop)) {
      if (stop < dlbuf->upstream_size) {
                printf("we hit the end, see what to do %ld,%ld,%ld \n", start,stop,dlbuf->upstream_size);
    ...
    if (available > threshold) {
                printf("further than threshold, it's better to skip than to reread \n");
    ...
    GstQuery * query)
{
  ...
  switch (GST_QUERY_TYPE (query)) {
    ...
    {
                    printf("(gst_download_buffer_handle_src_query) GST_QUERY_POSITION\n");
    ...
    {
              printf("(gst_download_buffer_handle_src_query) GST_QUERY_DURATION\n");
    ...
    {
              printf("(gst_download_buffer_handle_src_query) GST_QUERY_BUFFERING\n");
      gint percent;
...

      {
        ...
        GST_DEBUG_OBJECT (dlbuf, "percent %d, duration %" G_GINT64_FORMAT
            ", writing %" G_GINT64_FORMAT, percent, duration, write_pos);
                    ...
                    printf("(gst_download_buffer_handle_src_query) percent %d, duration %" G_GINT64_FORMAT ", writing %" G_GINT64_FORMAT "\n",
                    percent,
                    duration,
                    write_pos);
        ...
        while (gst_sparse_file_get_range_after (dlbuf->file, offset,
          ...
          GST_DEBUG_OBJECT (dlbuf,
              ...
              G_GSIZE_FORMAT, range_start, range_stop);
                  ...
                  printf("(gst_download_buffer_handle_src_query) range starting at %" G_GSIZE_FORMAT 
                  " and finish at %" G_GSIZE_FORMAT " \n", range_start, range_stop);
    ...
    GstPadMode mode, gboolean active)
{
  ...
  switch (mode) {
    case GST_PAD_MODE_PULL:
              ...
              printf("(gst_download_buffer_src_activate_mode)GST_PAD_MODE_PULL \n");

      res = gst_download_buffer_src_activate_pull (pad, parent, active);
      break;
    case GST_PAD_MODE_PUSH:
    
              printf("(gst_download_buffer_src_activate_mode)GST_PAD_MODE_PUSH \n");
