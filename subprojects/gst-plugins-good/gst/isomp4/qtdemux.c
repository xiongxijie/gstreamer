...
gst_qtdemux_pull_atom (GstQTDemux * qtdemux, guint64 offset, guint64 size,
    GstBuffer ** buf)
{
                                    // printf("(qtdemux_pull_atom) offset=%ld size=%ld\n", offset, size);
  ...
  if (G_UNLIKELY (size > QTDEMUX_MAX_ATOM_SIZE)) {
    if (qtdemux->state != QTDEMUX_STATE_MOVIE && qtdemux->got_moov) {
      ...
      return GST_FLOW_ERROR;
                                    ...
                                    printf("(qtdemux_pull_atom) gst_pad_pull_range offset=%ld size=%ld\n", offset, size);
    ...
    gint64 * dest_value)
{
              ...
              printf("(gst_qtdemux_src_convert)\n");
          ...
          *dest_value = stream->samples[index].offset;
          ...
          GST_DEBUG_OBJECT (qtdemux, "Format Conversion Time->Offset :%"
              ...
              GST_TIME_ARGS (src_value), *dest_value);
                        ...
                        printf("(qtdemux_src_convert), Time->Offset: %" GST_TIME_FORMAT " -> %" G_GUINT64_FORMAT " \n", GST_TIME_ARGS (src_value), *dest_value);
          break;
        }
        default:
          res = FALSE;
    ...
    GstQuery * query)
{
                          ...
                          printf("(qtdemux_handle_src_query)  %s\n",GST_QUERY_TYPE_NAME (query));
...
gst_qtdemux_push_tags (GstQTDemux * qtdemux, QtDemuxStream * stream)
{
  if (G_LIKELY (stream->pad)) {
                          ...
                          printf("(gst_qtdemux_push_tags) Checking pad %s:%s for tags\n"
  ...
  for (i = 0; i < QTDEMUX_N_STREAMS (qtdemux); i++) {
    ...
    if ((pad = stream->pad)) {
      ...
      if (etype == GST_EVENT_EOS) {
              printf("(qtdemux_push_event) GST_EVENT_EOS \n");
    ...
    QtDemuxStream * str, gint64 media_offset)
{
        ...
        printf("(gst_qtdemux_find_index_for_given_media_offset_linear) \n");
    ...
    GstClockTime media_time)
{
  ...
  mov_time =
      gst_util_uint64_scale_ceil (media_time, str->timescale, GST_SECOND);
                                                ...
                                                printf("(qtdemux_find_index_linear) convert media_time (%" GST_TIME_FORMAT ") to mov format, mov_time=%ld,timescale=%u \n", 
                                                GST_TIME_ARGS(media_time), mov_time, str->timescale);
  sample = str->samples;
  ...
  sample = str->samples + str->stbl_index;
                                                //   printf("(qtdemux_find_index_linear) sample->timestamp=%ld,sample->pts_offset=%ld str->stbl_idx=%d \n", 
                                                // sample->timestamp, sample->pts_offset ,str->stbl_index);
  ...
  while (index > 0 && sample->timestamp + sample->pts_offset > mov_time) {
    ...
    sample = str->samples + index;
                                          ...
                                          printf("(qtdemux_find_index_linear) (%s) found sample index %u at %" GST_TIME_FORMAT " \n", str->pad ? GST_PAD_NAME (str->pad) : "(NULL)", index, GST_TIME_ARGS(media_time));
...
beach:
  GST_DEBUG_OBJECT (qtdemux, "searching for keyframe index %s index %u "
      "gave %u", next ? "after" : "before", index, new_index);
                              ...
                              printf("(qtdemux_find_keyframe) find_keyframe, (%s) index %s index %u gave %u \n",str->pad ? GST_PAD_NAME (str->pad) : "(NULL)", next ? "after" : "before", index, new_index);
  ...
  for (i = 0; i < stream->n_segments; i++) {
    ...
    if (i < stream->n_segments - 1) {
      ...
      break;
                                ...
                                printf("(qtdemux_find_segment) (%s) find segment for %" GST_TIME_FORMAT ", found %d \n", stream->pad ? GST_PAD_NAME (stream->pad) : "(NULL)" ,GST_TIME_ARGS (time_position), seg_idx);
    ...
    guint32 index)
{
                                ...
                                printf("(qtdemux_move_stream) (%s) moving to sample %u of %u \n", str->pad ? GST_PAD_NAME (str->pad) : "(NULL)", index, str->n_samples);
...
gst_qtdemux_do_push_seek (GstQTDemux * qtdemux, GstPad * pad, GstEvent * event)
{
  ...
  gst_qtdemux_adjust_seek (qtdemux, cur, FALSE, FALSE, &key_cur, &byte_cur);
                                        ...
                                        printf("(qtdemux_do_push_seek) corresponding BYTE position %ld \n", byte_cur);
  ...
  GST_OBJECT_UNLOCK (qtdemux);
                                          ...
                                          printf("(qtdemux_do_push_seek) push BYTE seek event, cur_type:%u, stop_type:%u, cur:%ld. stop:%ld \n\n", cur_type,stop_type, byte_cur, stop);
    ...
    guint32 seqnum, GstSeekFlags flags)
{
  ...
  GST_DEBUG_OBJECT (qtdemux, "seeking to %" GST_TIME_FORMAT,
      GST_TIME_ARGS (desired_offset));
                                  ...
                                  printf("(qtdemux_perform_seek) seeking to %" GST_TIME_FORMAT " \n", GST_TIME_ARGS (desired_offset));
  ...
  if ((flags & GST_SEEK_FLAG_KEY_UNIT) && !qtdemux->fragmented) {
    ...
    desired_offset = min_offset;
                  printf(" in perform_seek, keyframe seek, align to %" GST_TIME_FORMAT "\n", GST_TIME_ARGS (min_offset));
  }
...
{
                          printf("(qtdemux_do_seek) do a seek in pull based mode \n");
  ...
  seqnum = gst_event_get_seqnum (event);
                                      ...
                                      printf("(qtdemux_do_seek) seqnum %u , cur=%ld, stop=%ld, cur_type=%d \n", seqnum, cur, stop, cur_type);
  ...
  GST_DEBUG_OBJECT (qtdemux, "seek format %s", gst_format_get_name (format));
                                      ...
                                      printf("(qtdemux_do_seek) seek format %s \n", gst_format_get_name (format));
  ...
  if (flush) {
                    printf("(qtdemux_do_seek) flushing pipeline \n");
  ...
  if (!gst_segment_do_seek (&seeksegment, rate, format, flags,
    ...
    /* now do the seek */
                                      ...
                                      printf("(qtdemux_do_seek) seeksegment, start=%ld, stop=%ld, position=%ld, time=%ld \n",
                                        seeksegment.start, seeksegment.stop, seeksegment.position, seeksegment.time);
  ...
  if (flush) {
              // printf("(qtdemux_do_seek) do_seek, by flushing pipeline, prepare for streaming again\n");
    ...
    GstEvent * event)
{
  ...
  GstQTDemux *qtdemux = GST_QTDEMUX (parent);
                                  ...
                                  // printf("(qtdemux_handle_src_event) %s \n", GST_EVENT_TYPE_NAME(event));
  ...
  switch (GST_EVENT_TYPE (event)) {
    ...
    case GST_EVENT_SEEK:
    {
      ...
      if (qtdemux->pullbased) {
                            ...
                            printf("(qtdemux_handle_src_event) do_seek in pull mode \n");
    ...
    gboolean set, QtDemuxStream ** _stream, gint * _index, gint64 * _time)
{
                          ...
                          printf("(gst_qtdemux_find_sample) \n");
...
_create_stream (GstQTDemux * demux, guint32 track_id)
{
  ...
  stream->stream_id = g_strdup_printf ("%s/%03u", upstream_id, track_id);
                                      ...
                                      printf("(_create_stream), stream_id=%s \n", stream->stream_id);
...
gst_qtdemux_map_and_push_segments (GstQTDemux * qtdemux, GstSegment * segment)
{
                      ...
                      printf("(gst_qtdemux_map_and_push_segments) \n");
    ...
    GstEvent * event)
{
  ...
  GST_LOG_OBJECT (demux, "handling %s event", GST_EVENT_TYPE_NAME (event));
                      ...
                      // printf("(qtdemux_handle_sink_event) %s \n", 
                      //       GST_EVENT_TYPE_NAME (event));
  ...
  switch (GST_EVENT_TYPE (event)) {
    ...
    {
            printf("(qtdemux_handle_sink_event) got GST_EVENT_FLUSH_START \n");
    ...
    {
            printf("(qtdemux_handle_sink_event) got GST_EVENT_FLUSH_STOP \n");
      ...
      if (!demux->pullbased) {
        ...
        if (!has_valid_stream)
        {
          gst_qtdemux_post_no_playable_stream_error (demux);
                  printf("(qtdemux_handle_sink_event) post_no_playable_stream_error\n");

        }
        else {
                  printf("(qtdemux_handle_sink_event) in push mode, Data still available after EOS: %u\n",
    ...
    case GST_EVENT_STREAM_START:
    {
                                          ...
                                          printf("(qtdemux_handle_sink_event) GST_EVENT_STREAM_START \n");
    ...
    GstQuery * query)
{
              ...
              // printf("(qtdemux_handle_sink_query) %s\n", GST_QUERY_TYPE_NAME (query));
...
gst_qtdemux_stream_flush_samples_data (QtDemuxStream * stream)
{
              ...
              printf("(gst_qtdemux_stream_flush_samples_data) \n");
    ...
    guint64 * plength, guint32 * pfourcc)
{
  ...
  GST_DEBUG ("atom type %" GST_FOURCC_FORMAT, GST_FOURCC_ARGS (fourcc));
                              ...
                              // printf("(extract_initial_length_and_fourcc) atom type is %" GST_FOURCC_FORMAT  " length=%ld \n", GST_FOURCC_ARGS (fourcc),length);
      ...
      while (trex) {
        ...
        GST_DEBUG_OBJECT (qtdemux, "fragment defaults for stream %d; "
            ...
            dur, size, flags);
                    ...
                    printf("(qtdemux_parse_trex) %d %d %d\n", 
                    stream->track_id, dur, size);
    ...
    gboolean has_tfdt, guint trun_node_total)
{
  ...
  GST_LOG_OBJECT (qtdemux, "parsing trun track-id %d; "
      ...
      d_sample_size, d_sample_flags, *base_offset, decode_ts);
                                            ...
                                            printf("(qtdemux_parse_trun) trun track-id %d, moof_offset=%ld,moof_len=%ld,base_offset=%ld,decode_ts=%ld \n", 
                                                stream->track_id, moof_offset, moof_length, *base_offset, decode_ts);
  ...
  GST_DEBUG_OBJECT (qtdemux, "allocating n_samples %u * %u (%.2f MB)",
      ...
      sizeof (QtDemuxSample) / (1024.0 * 1024.0));
                                    ...
                                    printf("(qtdemux_parse_trun) samples_count=%u stream->n_samples=%u, (%ld Bytes) \n", samples_count,stream->n_samples, (stream->n_samples + samples_count) *  sizeof (QtDemuxSample) );
  ...
  for (i = 0; i < samples_count; i++) {
    ...
    /* fill the sample information */
                                         ...
                                         printf("(qtdemux_parse_trun), fill sample info, running_offset=%ld \n", *running_offset);
    ...
    guint64 moof_offset, QtDemuxStream * stream)
{
  ...
  GST_DEBUG_OBJECT (qtdemux, "Fragment #%d", frag_num);
                          ...
                          printf("(qtdemux_parse_moof) moof NO.%d ,moof-offset is%ld\n", frag_num, moof_offset);
  ...
  while (traf_node) {
    ...
    if (!qtdemux_parse_tfhd (qtdemux, &tfhd_data, &stream, &ds_duration,
      ...
      goto missing_tfhd;
                            ...
                            // printf("(qtdemux_parse_moof) parse tfhd, %d %d \n",ds_duration, ds_size);
...
gst_qtdemux_loop_state_header (GstQTDemux * qtdemux)
{
  ...
  GstMapInfo map;
                                ...
                                printf("(qtdemux_loop_state_header) gst_pad_pull_range, offset=%ld ,len=16\n", cur_offset);
    ...
    case FOURCC_moof:
      ...
      if (qtdemux_pull_mfro_mfra (qtdemux)) {
        /* FIXME */
                    ...
                    printf("(qtdemux_loop_state_header) got moof, skip moof qtdemux->offset += %ld \n", length);
      ...
      if (qtdemux->got_moov) {
        GST_INFO_OBJECT (qtdemux, "moof header, got moov, done with headers");
                        ...
                        printf("(qtdemux_loop_state_header) moof header, got moov, done with headers\n");
    ...
    case FOURCC_pnot:
    {
      GST_LOG_OBJECT (qtdemux,
          ...
          GST_FOURCC_ARGS (fourcc), cur_offset);
                        ...
                        printf("(qtdemux_loop_state_header) skip atom %" GST_FOURCC_FORMAT " at %" G_GUINT64_FORMAT "\n",
                        GST_FOURCC_ARGS (fourcc), cur_offset);
    ...
    case FOURCC_moov:
    {
      ...
      if (length == G_MAXUINT64) {
        ...
        if (length > QTDEMUX_MAX_ATOM_SIZE) {
          ...
          goto beach;
                                ...
                                printf("(qtdemux_loop_state_header) try [moov] gst_pad_pull_range, offset=%ld, len=%ld\n", cur_offset, length);
      ...
      if (length != map.size) {
        ...
        goto beach;
                                      ...
                                      printf("(qtdemux_loop_state_header) got [moov], qtdemux->offset += %ld \n", length);
    ...
    case FOURCC_ftyp:
    {
      ...
      if (ret != GST_FLOW_OK)
        goto beach;
                    ...
                    printf("(qtdemux_loop_state_header) got [ftyp], qtdemux->offset += %ld \n", length);
    ...
    default:
    {
      ...
      GST_LOG_OBJECT (qtdemux,
          ...
          cur_offset);
                                          ...
                                          printf("(qtdemux_loop_state_header) Unknow atom %" GST_FOURCC_FORMAT ", curoffset=%ld \n",GST_FOURCC_ARGS (fourcc) ,cur_offset);
      ...
      gst_buffer_unref (unknown);
                                          printf("(qtdemux_loop_state_header) Unknow atom %" GST_FOURCC_FORMAT ", qtdemux->offset += %ld \n",GST_FOURCC_ARGS (fourcc) ,length);
  ...
  if (ret == GST_FLOW_EOS && (qtdemux->got_moov || qtdemux->media_caps)) {
    ...
    GST_DEBUG_OBJECT (qtdemux, "switching state to STATE_MOVIE (%d)",
        qtdemux->state);
              ...
              printf("(qtdemux_loop_state_header) switching state to STATE_MOVIE \n");
    return ret;
  }
  return ret;
}
...
gst_qtdemux_seek_to_previous_keyframe (GstQTDemux * qtdemux)
{
                          ...
                          printf("(gst_qtdemux_seek_to_previous_keyframe) \n");
    ...
    GstClockTime * _start, GstClockTime * _stop, GstClockTime * _time)
{
  ...
  GST_LOG_OBJECT (stream->pad, "seg_time %" GST_TIME_FORMAT,
      GST_TIME_ARGS (seg_time));
                                        ...
                                        printf("(stream_segment_get_boundaries) (%s) seg_time = %" GST_TIME_FORMAT " \n", stream->pad ? GST_PAD_NAME (stream->pad) : "(NULL)", GST_TIME_ARGS (seg_time));
  ...
  if (G_UNLIKELY (seg_time > segment->duration)) {
    GST_LOG_OBJECT (stream->pad,
        ...
        GST_TIME_ARGS (segment->duration));
                                        ...
                                        printf("(stream_segment_get_boundaries) (%s) seg_time > segment->duration %" GST_TIME_FORMAT " \n", stream->pad ? GST_PAD_NAME (stream->pad) : "(NULL)" ,GST_TIME_ARGS (segment->duration));
  ...
  if (G_UNLIKELY (QTSEGMENT_IS_EMPTY (segment))) {
    ...
    stop = MIN (segment->media_start + seg_time, stop);
                                        ...
                                        printf("(stream_segment_get_boundaries) (%s) result is start=%" GST_TIME_FORMAT ",stop=%" GST_TIME_FORMAT ",time=%" GST_TIME_FORMAT "\n", 
                                              stream->pad ? GST_PAD_NAME (stream->pad) : "(NULL)", GST_TIME_ARGS (start),GST_TIME_ARGS (stop),GST_TIME_ARGS (time) );
    ...
    GstClockTime * _stop)
{
  ...
  GST_DEBUG_OBJECT (stream->pad, "new segment %d from %" GST_TIME_FORMAT
      ...
      GST_TIME_ARGS (start), GST_TIME_ARGS (stop), GST_TIME_ARGS (time));
                      ...
                      printf("(qtdemux_stream_update_segment) (%s) new segment %d from %" GST_TIME_FORMAT " to %" GST_TIME_FORMAT ",time %" GST_TIME_FORMAT " \n", 
                          stream->pad ? GST_PAD_NAME (stream->pad) : "(NULL)",seg_idx,GST_TIME_ARGS (start), GST_TIME_ARGS (stop), GST_TIME_ARGS (time));
    ...
    guint32 seg_idx, GstClockTime offset)
{
  ...
  GST_LOG_OBJECT (stream->pad, "activate segment %d, offset %" GST_TIME_FORMAT,
      seg_idx, GST_TIME_ARGS (offset));
                                        ...
                                        printf("(qtdemux_activate_segment) (%s) activate segment %d, offset %" GST_TIME_FORMAT "\n", stream->pad ? GST_PAD_NAME (stream->pad) : "(NULL)", seg_idx, GST_TIME_ARGS (offset));
  ...
  if (index == stream->sample_index) {
                        printf("(qtdemux_activate_segment) we are at the right index\n");
  ...
  if (index > stream->sample_index) {
    ...
    if (kf_index > stream->sample_index) {
      GST_DEBUG_OBJECT (stream->pad,
          ...
          GST_TIME_ARGS (QTSAMPLE_DTS (stream, &stream->samples[kf_index])));
                                                      ...
                                                      printf("(qtdemux_activate_segment) (%s) moving forwards to keyframe at %u, (pts %" GST_TIME_FORMAT " dts %" GST_TIME_FORMAT " ) \n", 
                                                          stream->pad ? GST_PAD_NAME (stream->pad) : "(NULL)", kf_index,
                                                          GST_TIME_ARGS (QTSAMPLE_PTS (stream, &stream->samples[kf_index])),
                                                          GST_TIME_ARGS (QTSAMPLE_DTS (stream, &stream->samples[kf_index]))
                                                      );
      ...
      GST_DEBUG_OBJECT (stream->pad,
          ...
          GST_TIME_ARGS (QTSAMPLE_DTS (stream, &stream->samples[kf_index])));
                                                        ...
                                                        printf("(qtdemux_activate_segment) (%s) moving forwards, keyframe at %u (pts %" GST_TIME_FORMAT " dts %" GST_TIME_FORMAT " ) already sent\n", 
                                                            stream->pad ? GST_PAD_NAME (stream->pad) : "(NULL)", kf_index,
                                                            GST_TIME_ARGS (QTSAMPLE_PTS (stream, &stream->samples[kf_index])),
                                                            GST_TIME_ARGS (QTSAMPLE_DTS (stream, &stream->samples[kf_index]))
                                                          );
    ...
    GST_DEBUG_OBJECT (stream->pad,
        ...
        GST_TIME_ARGS (QTSAMPLE_DTS (stream, &stream->samples[kf_index])));
                                                        ...
                                                        printf("(qtdemux_activate_segment) (%s) moving backwards to %sframe at %u (pts %" GST_TIME_FORMAT " dts %" GST_TIME_FORMAT " )\n", 
                                                            stream->pad ? GST_PAD_NAME (stream->pad) : "(NULL)", (stream->subtype == FOURCC_soun) ? "audio " : "key", kf_index,
                                                            GST_TIME_ARGS (QTSAMPLE_PTS (stream, &stream->samples[kf_index])),
                                                            GST_TIME_ARGS (QTSAMPLE_DTS (stream, &stream->samples[kf_index])));
  ...
  *empty = FALSE;
  ...
  if (G_UNLIKELY (stream->sample_index >= stream->n_samples)) {
    ...
    GST_INFO_OBJECT (qtdemux, "out of samples, trying to add more");
                                ...
                                printf("(qtdemux_prepare_current_sample) out of samples, trying add more \n");
    ...
    while (stream->sample_index >= stream->n_samples);
                                  ...
                                  printf("(qtdemux_prepare_current_sample) pad=%s segment active, index = %u of %u\n", 
                                      stream->pad ? GST_PAD_NAME (stream->pad) : "(NULL)" ,stream->sample_index, stream->n_samples);
...
gst_qtdemux_advance_sample (GstQTDemux * qtdemux, QtDemuxStream * stream)
{
  ...
  segment = &stream->segments[stream->segment_index];
                    ...
                    printf("(qtdemux_advance_sample) stream->segment_index = %d, stream->n_segments = %d \n", stream->segment_index, stream->n_segments);
  ...
  if (G_UNLIKELY (stream->sample_index >= stream->n_samples))
    goto next_segment;
                            ...
                            printf("(qtdemux_advance_sample) pad=%s  >  advance to sample %u/%u\n", stream->pad ? GST_PAD_NAME (stream->pad) : "(NULL)" , stream->sample_index, stream->n_samples);
  ...
  GST_TRACE_OBJECT (qtdemux, "sample dts %" GST_TIME_FORMAT " media_stop: %"
      ...
      GST_TIME_ARGS (segment->media_stop));
                          ...
                          // printf("(qtdemux_advance_sample), sample dts %" GST_TIME_FORMAT "  media_stop: %" GST_TIME_FORMAT " \n", GST_TIME_ARGS (QTSAMPLE_DTS (stream, sample)), GST_TIME_ARGS (segment->media_start));
...
gst_qtdemux_sync_streams (GstQTDemux * demux)
{
                                ...
                                printf("(qtdemux_sync_streams) num streams is %u\n", QTDEMUX_N_STREAMS (demux));
  ...
  for (i = 0; i < QTDEMUX_N_STREAMS (demux); i++) {
    ...
    GST_LOG_OBJECT (demux, "current position: %" GST_TIME_FORMAT
        ...
        GST_TIME_ARGS (demux->segment.position), GST_TIME_ARGS (end_time));
                                ...
                                printf("(qtdemux_sync_stream) (%s) current position: %" GST_TIME_FORMAT
                                ", stream end: %" GST_TIME_FORMAT " \n", stream->pad ? GST_PAD_NAME (stream->pad) : "(NULL)",GST_TIME_ARGS (demux->segment.position), GST_TIME_ARGS (end_time));
    ...
    if (GST_CLOCK_TIME_IS_VALID (end_time)
      ...
      GST_DEBUG_OBJECT (demux, "sending EOS for stream %s",
          GST_PAD_NAME (stream->pad));
                                  ...
                                  printf("(qtdemux_sync_streams) sending EOS for stream %s\n", GST_PAD_NAME (stream->pad));
    ...
    GstBuffer * buf)
{
  ...
  GST_LOG_OBJECT (qtdemux,
      ...
      GST_TIME_ARGS (GST_BUFFER_DURATION (buf)), GST_PAD_NAME (stream->pad));
                              ...
                              printf("(qtdemux_push_buffer) (%s) Pushing buffer on pad %s\n",
                               stream->pad ? GST_PAD_NAME (stream->pad) : "(NULL)" , GST_PAD_NAME (stream->pad) );
  ...
  duration = GST_BUFFER_DURATION (buf);
                                    ...
                                    printf("(qtdemux_push_buffer) gst_pad_push(stream->pad, buf) \n");
...
{
                                              printf("(qtdemux_decorate_and_push_buffer) (%s) %" GST_TIME_FORMAT ",byte pos=%ld \n", stream->pad ? GST_PAD_NAME (stream->pad) : "(NULL)", GST_TIME_ARGS(position), byte_position);
  ...
  while (stream->buffers) {
    ...
    if (stream->alignment > 1)
      buffer = gst_qtdemux_align_buffer (qtdemux, buffer, stream->alignment);
                        printf("(qtdemux_decorate_and_push_buffer) gst_pad_push(stream->pad, buffer) \n");
...
{
                  printf("(qtdemux_do_fragmented_seek) \n");
  ...
  qtdemux->moof_offset = best_entry->moof_offset;
                                ...
                                printf("(qtdemux_do_fragmented_seek), seek to %" GST_TIME_FORMAT ", best fragment " "moof offset: %" G_GUINT64_FORMAT "\n"
                                ,GST_TIME_ARGS (QTDEMUX_NTH_STREAM (qtdemux, 0)->time_position), best_entry->moof_offset);
...
gst_qtdemux_loop_state_movie (GstQTDemux * qtdemux)
{
  ...
  for (i = 0; i < QTDEMUX_N_STREAMS (qtdemux); i++) {
    ...
    stream = QTDEMUX_NTH_STREAM (qtdemux, i);
                          printf("(loop_state_movie)  %" GST_TIME_FORMAT " \n", GST_TIME_ARGS(stream->time_position));
    ...
    if (position != GST_CLOCK_TIME_NONE && position < min_time) {
      min_time = position;
                          ...
                          printf("(loop_state_movie) target_stream=stream %" GST_FOURCC_FORMAT " \n", GST_FOURCC_ARGS (CUR_STREAM (stream)->fourcc));
  ...
  GST_DEBUG_OBJECT (qtdemux,
      ...
      GST_TIME_ARGS (duration));
                                    ...
                                    // printf("(loop_state_movie) pushing offset %" G_GUINT64_FORMAT" ,duration= %" GST_TIME_FORMAT " \n", offset , GST_TIME_ARGS (duration));
  ...
  GST_LOG_OBJECT (qtdemux, "reading %d bytes @ %" G_GUINT64_FORMAT, size,
      offset);
                                                ...
                                                printf("(loop_state_movie) reading %d bytes at offset %" G_GUINT64_FORMAT "\n", size, offset);
  ...
  if (ret == GST_FLOW_EOS)
    ret = GST_FLOW_OK;
                                      ...
                                      printf("(loop_state_movie) offset_in_sample=%d, size=%ld \n",stream->offset_in_sample, size);
...
gst_qtdemux_loop (GstPad * pad)
{
  ...
  GST_LOG_OBJECT (qtdemux, "loop at position %" G_GUINT64_FORMAT ", state %s",
      cur_offset, qt_demux_state_string (qtdemux->state));
                    ...
                    printf("[qtdemux_loop] loop at position %" G_GUINT64_FORMAT " state %s \n", cur_offset, qt_demux_state_string (qtdemux->state));
...
pause:
  {
    ...
    GST_LOG_OBJECT (qtdemux, "pausing task, reason %s", reason);
                            ...
                            printf("[qtdemux_loop] pausing task reason %s \n", reason);
    ...
    if (ret == GST_FLOW_EOS) {
      ...
      if (qtdemux->segment.flags & GST_SEEK_FLAG_SEGMENT) {
        ...
        GstEvent *event;
                printf ("(qtdemux_loop) Sending EOS at end of segment \n");
...
has_next_entry (GstQTDemux * demux)
{
  QtDemuxStream *stream;
  gint i;
                  printf("(has_next_entry) Checking if there are samples not played yet\n");
...
next_entry_size (GstQTDemux * demux)
{
  ...
  QtDemuxSample *sample;
  gint i;
                        printf("(next_entry_size) demux->offset is %" G_GUINT64_FORMAT " \n", demux->offset);
  ...
  for (i = 0; i < QTDEMUX_N_STREAMS (demux); i++) {
    ...
    if (!qtdemux_parse_samples (demux, stream, stream->sample_index)) {
      GST_LOG_OBJECT (demux, "Parsing of index %u from stbl atom failed!",
          stream->sample_index);
                // printf("(next_entry_size) Parseing of index %u from stbl atom failed \n");
      return -1;
    }
    ...
    GST_LOG_OBJECT (demux,
        ...
        stream->sample_index, sample->offset, sample->size);
              ...
              // printf("(next_entry_size) sample_index:%d / offset:%" G_GUINT64_FORMAT " / size:%" G_GUINT32_FORMAT ")\n", stream->sample_index, sample->offset, sample->size);
  ...
  GST_LOG_OBJECT (demux,
      ...
      G_GUINT64_FORMAT, target_stream->track_id, smalloffs, demux->offset);
              ...
              // printf("(next_entry_size) track-id %u offset %" G_GUINT64_FORMAT " demux->offset:%" G_GUINT64_FORMAT "\n", 
              // target_stream->track_id, smalloffs, demux->offset);
  ...
  GST_DEBUG_OBJECT (demux,
      "There wasn't any entry at offset %" G_GUINT64_FORMAT, demux->offset);
              ...
              // printf("(next_entry_size) There wasn't any entry at offset %" G_GUINT64_FORMAT "\n", demux->offset);
...
qtdemux_seek_offset (GstQTDemux * demux, guint64 offset)
{
  ...
  GST_DEBUG_OBJECT (demux, "Seeking to %" G_GUINT64_FORMAT, offset);
                ...
                printf("(qtdemux_seek_offset) Seeking to %" G_GUINT64_FORMAT "\n", offset);
...
gst_qtdemux_check_seekability (GstQTDemux * demux)
{
  ...
  gst_query_unref (query);
                  ...
                  printf("(gst_qtdemux_check_seekability) seekable:%d, start:%" G_GUINT64_FORMAT " stop:%" G_GUINT64_FORMAT "\n",
                  seekable, start, stop);
...
gst_qtdemux_drop_data (GstQTDemux * demux, gint bytes)
{
                          ...
                          printf("(qtdemux_drop_data) dropping %d bytes \n", bytes);
...
gst_qtdemux_check_send_pending_segment (GstQTDemux * demux)
{
                          ...
                          printf("(gst_qtdemux_check_send_pending_segment) \n");
...
gst_qtdemux_chain (GstPad * sinkpad, GstObject * parent, GstBuffer * inbuf)
{
  ...
  GST_DEBUG_OBJECT (demux,
      ...
      gst_buffer_get_size (inbuf), demux->offset);
                      ...
                      printf("(qtdemux_chain) Received buffer size:%" G_GSIZE_FORMAT " demux offset:%" G_GUINT64_FORMAT "\n"
                      ,  gst_buffer_get_size (inbuf), demux->offset);
        ...
        if (res != -1) {
          ...
          if (sample->offset == GST_BUFFER_OFFSET (inbuf)) {
            ...
            /* Finally update all push-based values to the expected values */
                            ...
                            printf("(qtdemux_chain) demux->neededbytes = stream->samples[res].size %u\n", 
                                    stream->samples[res].size);
  ...
  GST_DEBUG_OBJECT (demux,
      ...
      demux->neededbytes, gst_adapter_available (demux->adapter));
                              ...
                              printf("(qtdemux_chain) pushing in inbuf %p,neededbytes:%u,available: %" G_GSIZE_FORMAT "\n",
...
{       
                  ...
                  printf("(gst_qtdemux_process_adapter) demux->state=%s \n",qt_demux_state_string (demux->state));
...
#ifndef GST_DISABLE_GST_DEBUG
    {
      ...
      GST_DEBUG_OBJECT (demux,
          ...
          demux->offset, discont_offset, distance_from_discont);
                          ...
                          printf("(qtdemux_process_adapter) state:%s, demux->neededbytes:%d, demux->offset:%" G_GUINT64_FORMAT " (+ %" G_GUINT64_FORMAT " bytes)\n",qt_demux_state_string (demux->state), demux->neededbytes, demux->offset,distance_from_discont);
    }
    ...
    switch (demux->state) {
      case QTDEMUX_STATE_INITIAL:{
        ...
        GST_DEBUG_OBJECT (demux, "Peeking found [%" GST_FOURCC_FORMAT "] "
            "size: %" G_GUINT64_FORMAT, GST_FOURCC_ARGS (fourcc), size);
                                    ...
                                    printf("(qtdemux_process_adapter) InQTDEMUX_STATE_INITIAL, Peeking found [%" GST_FOURCC_FORMAT "] size:%" G_GUINT64_FORMAT "\n",
        ...
        if (size == 0) {
          GST_ELEMENT_ERROR (demux, STREAM, DEMUX,
              ...
              ("initial atom '%" GST_FOURCC_FORMAT "' has empty length",
                  GST_FOURCC_ARGS (fourcc)));
                                        printf("(qtdemux_process_adapter) This file is invalid and cannot be played,initial atom '%" GST_FOURCC_FORMAT "' has empty length\n",
                                        GST_FOURCC_ARGS (fourcc));
        ...
        if (fourcc == FOURCC_mdat) {
          gint next_entry = next_entry_size (demux);
                        ...
                        if(demux->got_moov)
                        {
                              printf("(qtdemux_process_adapter) FOURCC_mdat.  already got moov header\n");
          ...
          if (QTDEMUX_N_STREAMS (demux) > 0 && (next_entry != -1
            ...
            /* we have the headers, start playback */
                                          ...
                                          printf("(qtdemux_process_adapter) FOURCC_mdat. we have the headers, start playback, state set to QTDEMUX_STATE_MOVIE, and set neededbytes to %u\n", next_entry);
            ...
            /* no headers yet, try to get them */
                                          ...
                                          printf("(qtdemux_process_adapter) FOURCC_mdat. no headers yet, try to get them\n");
          ...
          buffer_data:
            ...
            if (demux->upstream_seekable &&
              ...
              res = qtdemux_seek_offset (demux, target);
                                          ...
                                          printf("(qtdemux_process_adaoter) try to jump over the atom with a seek, skipping seek\n");
            ...
            if (res) {
              GST_DEBUG_OBJECT (demux, "seek success");
                                          ...
                                          printf("(qtdemux_process_adaoter) seek success\n");
              ...
              if (fourcc == FOURCC_mdat && demux->first_mdat == -1) {
                ...
                GST_DEBUG_OBJECT (demux, "first mdat at %" G_GUINT64_FORMAT,
                    demux->first_mdat);
                                          ...
                                          printf("(qtdemux_process_adaoter) first mdat at %" G_GUINT64_FORMAT "\n",
                                          demux->first_mdat);
              ...
              GST_DEBUG_OBJECT (demux, "seek failed/skipped");
                                          ...
                                          printf("(qtdemux_process_adapter) seek failed/skipped : %" G_GUINT64_FORMAT "\n", demux->offset);
              ...
              if (size + bs > 10 * (1 << 20))
                goto no_moov;
                              ...
                              printf("(qtdemux_process_adapter) Clear data in adapter, demux->mdatoffset=%" G_GUINT64_FORMAT "\n",demux->offset);
                              
                              demux->last_moov_offset = target;
                              if (fourcc == FOURCC_mdat && demux->first_mdat == -1) {
                                  demux->first_mdat = demux->offset;
                              }
                              gst_adapter_clear (demux->adapter);
                              demux->state = QTDEMUX_STATE_INITIAL;    
                              demux->neededbytes = 16;
              ...
              if (!demux->mdatbuffer)
              {
                        printf("(qtdemux_process_adapter) !demux->mdatbuffer\n");
                demux->mdatoffset = demux->offset;
              }
              */
          ...
          GST_ELEMENT_ERROR (demux, STREAM, DEMUX,
              ...
              ("atom %" GST_FOURCC_FORMAT " has bogus size %" G_GUINT64_FORMAT,
                  GST_FOURCC_ARGS (fourcc), size));
                            ...
                            printf("(qtdemux_process_adapter)This file is invalid and cannot be played. atom %" GST_FOURCC_FORMAT " have bogus size %" G_GUINT64_FORMAT " \n",
                            GST_FOURCC_ARGS (fourcc), size);
           ...
           * let's continue buffering everything till we get moov */
                                        ...
                                        printf("(qtdemux_process_adapter) already buffering still no moov header, set state to QTDEMUX_STATE_HEADER, set demux->neededbytes=%" G_GUINT64_FORMAT "\n"
                                        ,size);
      ...
      case QTDEMUX_STATE_HEADER:{
        ...
        extract_initial_length_and_fourcc (data, demux->neededbytes, NULL,
            ...
            printf("(qtdemux_process_adapter) In QTDEMUX_STATE_HEADER, fourcc is %" GST_FOURCC_FORMAT "\n",
             GST_FOURCC_ARGS (fourcc));
        ...
        if (fourcc == FOURCC_moov) {
          ...
          if (demux->got_moov && QTDEMUX_N_STREAMS (demux) > 0 &&
            ...
            GST_DEBUG_OBJECT (demux,
                "Skipping moov atom as we have (this) one already");
                                              ...
                                              printf("(qtdemux_process_adapter) Skipping moov atom as we have (this) one already \n");

          } else {
            GST_DEBUG_OBJECT (demux, "Parsing [moov]");

                                              printf("(qtdemux_process_adapter) Parsing [moov] \n");
            ...
            demux->last_moov_offset = demux->offset;
                                              ...
                                              printf("(qtdemux_process_adapter) demux->last_moov_offset= %" G_GUINT64_FORMAT "\n",
                                              demux->last_moov_offset);
            ...
            GST_DEBUG_OBJECT (demux, "Finished parsing the header");
                              ...
                              printf("(qtdemux_process_adapter) Finished parsing the header \n");
          ...
          if ((demux->got_moov || demux->media_caps) && demux->fragmented) {
            ...
            GST_DEBUG_OBJECT (demux, "Parsing [moof]");
                                  ...
                                  printf("(process adaptor) Parsing [moof]\n");
        ...
        if (demux->mdatbuffer && QTDEMUX_N_STREAMS (demux)) {
          ...
          gst_adapter_flush (demux->adapter, demux->neededbytes);
                              ...
                              printf("(qtdemux_process_adapter) only go back to the mdat if there are samples to play first_mdat=%" G_GUINT64_FORMAT "\n",demux->first_mdat);

          /* only go back to the mdat if there are samples to play */
          if (demux->got_moov && demux->first_mdat != -1
              && has_next_entry (demux)) {

                              printf("(qtdemux_process_adapter) In QTDEMUX_STATE_HEADER, set demux->offset to demux->first_mdat(%" G_GUINT64_FORMAT ") \n", demux->first_mdat);

            demux->offset = demux->first_mdat;
     
            //below comment is official code, i comment them out for my use
            // gboolean res;
            ...
            // }
                              ...
                              printf("(qtdemux_process_adapter) In QTDEMUX_STATE_HEADER, set demux->offset+=demux->neededbytes(%" G_GUINT64_FORMAT ") \n", demux->neededbytes);
          ...
          demux->state = QTDEMUX_STATE_INITIAL;
                              ...
                              printf("(qtdemux_process_adapter) In QTDEMUX_STATE_HEADER, set demux->neededbytes=16 and demux->state back to QTDEMUX_STATE_INITIAL \n");
      ...
      case QTDEMUX_STATE_BUFFER_MDAT:{
        GstBuffer *buf;
        guint8 fourcc[4];
                      printf("(qtdemux_process_adapter)QTDEMUX_STATE_BUFFER_MDAT \n");
      ...
      case QTDEMUX_STATE_MOVIE:{
        ...
        GST_DEBUG_OBJECT (demux,
            "BEGIN // in MOVIE for offset %" G_GUINT64_FORMAT, demux->offset);
                                      ...
                                      printf(" BEGIN // in MOVIE for offset %" G_GUINT64_FORMAT "\n", demux->offset);
        ...
        for (i = 0; i < QTDEMUX_N_STREAMS (demux); i++) {
          ...
          GST_LOG_OBJECT (demux,
              ...
              stream->samples[stream->sample_index].size);
                      ...
                      printf("In QTDEMUX_SATTE_MOVIE, Checking track-id %u (sample_index:%d / offset:%" G_GUINT64_FORMAT " /size:%d) \n",
                      stream->track_id, stream->sample_index,stream->samples[stream->sample_index].offset, stream->samples[stream->sample_index].size);
        ...
        if (G_LIKELY (!(STREAM_IS_EOS (stream)))) {
          ...
          if (G_UNLIKELY (stream->segment.stop != -1
            ...
            stream->time_position = GST_CLOCK_TIME_NONE;        /* this means EOS */
                                    ...
                                    printf("In QTDEMUX_STATE_MOVIE, we reached the end of our segment. \n");
        ...
        GST_LOG_OBJECT (demux, "offset is now %" G_GUINT64_FORMAT,
            demux->offset);
                  printf("In QTDEMUX_STATE_MOVIE, increasing offset by %u, ofset now is %" G_GUINT64_FORMAT " \n",
                  demux->neededbytes, demux->offset);

        if (ret == GST_FLOW_EOS) {

                  printf("In QTDEMUX_STATE_MOVIE, All streams are EOS, signal upstream\n");
      ...
      default:
        goto invalid_state;
                                          ...
                                          printf("(qtdemux_process_adapter) out of while loop, available:%ld, neededbytes:%u, ret:%d \n",
                                          gst_adapter_available (demux->adapter), demux->neededbytes, ret);
...
eos:
  {
    GST_DEBUG_OBJECT (demux, "no next entry, EOS");
            printf ("(qtdemux_process_adapter) no next entry, EOS \n");
    ret = GST_FLOW_EOS;
    goto done;
  }
invalid_state:
...
no_moov:
  {
    GST_ELEMENT_ERROR (demux, STREAM, FAILED,
        (NULL), ("no 'moov' atom within the first 10 MB"));
                                    ...
                                    printf("(qtdemux_process_adapter) no moov atom within the first 10MB\n");
...
activate_push:
  {
                ...
                printf("(qtdemux_sink_activate) activating push \n");
    ...
    case GST_PAD_MODE_PULL:
      if (active) {
        demux->pullbased = TRUE;
                                  ...
                                  printf("(qtdemux_sink_activate_mode) GST_PAD_MODE_PULL\n");
...
qtdemux_parse_moov (GstQTDemux * qtdemux, const guint8 * buffer, guint length)
{
  ...
  GST_DEBUG_OBJECT (qtdemux, "parsing 'moov' atom");
                          ...
                          printf("(qtdemux_parse_moov)parsing moov atom!!! \n");
    ...
    guint length)
{
  ...
  GST_LOG_OBJECT (qtdemux,
      ...
      GST_FOURCC_ARGS (fourcc), node_length, type->name);
                              ...
                              // printf("(qtdemux_parse_node) parsing '%" GST_FOURCC_FORMAT "', length=%u, name= '%s' \n", 
                              //       GST_FOURCC_ARGS (fourcc), node_length, type->name);
...
{
  if (stream->subtype == FOURCC_vide) {
                  printf("(qtdemux_configure_stream) FOURCC_vide \n");
      ...
      if (stream->multiview_mode != GST_VIDEO_MULTIVIEW_MODE_NONE) {
        ...
        gst_caps_set_simple (CUR_STREAM (stream)->caps,
            ...
            stream->multiview_flags, GST_FLAG_SET_MASK_EXACT, NULL);
                  ...
                  printf("(qtdemux_configure_stream) FOURCC_soun \n");
  ...
  if (stream->pad) {
    ...
    GST_DEBUG_OBJECT (qtdemux, "adding pad %s %p to qtdemux %p",
        GST_OBJECT_NAME (stream->pad), stream->pad, qtdemux);
            ...
            printf ("(qtdemux_add_stream) adding pad %s \n", GST_OBJECT_NAME (stream->pad));
    ...
    guint64 * length, guint32 fourcc)
{
  ...
  GST_LOG_OBJECT (qtdemux, "finding fourcc %" GST_FOURCC_FORMAT " at offset %"
      G_GUINT64_FORMAT, GST_FOURCC_ARGS (fourcc), *offset);
                                  ...
                                  printf("(qtdemux_find_atom) finding fourcc %" GST_FOURCC_FORMAT " at offset %" G_GUINT64_FORMAT "\n",
...
qtdemux_add_fragmented_samples (GstQTDemux * qtdemux)
{
  ...
  GST_DEBUG_OBJECT (qtdemux, "next moof at offset %" G_GUINT64_FORMAT, offset);
                            ...
                            printf("(qtdemux_add_fragmented_samples), next moof at offset %ld\n", offset);
...
{
          printf("(qtdemux_merge_sample_table) \n");
  guint i;
  guint32 num_chunks;
...
qtdemux_stbl_init (GstQTDemux * qtdemux, QtDemuxStream * stream, GNode * stbl)
{
  ...
  if (!stream->n_samples)
    goto no_samples;
                                          ...
                                          printf("(qtdemux_stbl_init) [stsz] stream->sample_size=%d, stream->n_samples=%d \n", stream->sample_size, stream->n_samples);
  ...
  if (!qt_atom_parser_has_chunks (&stream->stsc, stream->n_samples_per_chunk,
    ...
    goto corrupt_file;
                                                    ...
                                                    printf("(qtdemux_stbl_init), [stsc] stream->n_samples_per_chunk=%d \n", stream->n_samples_per_chunk);
  ...
  GST_DEBUG_OBJECT (qtdemux, "allocating n_samples %u * %u (%.2f MB)",
      ...
      stream->n_samples * sizeof (QtDemuxSample) / (1024.0 * 1024.0));
                            ...
                            printf("(qtdemux_stbl_init) allocating n_samples %u, (guint)sizeof(QtDemuxSample) is %u\n", 
                                    stream->n_samples, (guint) sizeof (QtDemuxSample));
...
corrupt_file:
  {
                          ...
                          printf("(qtdemux_stbl_init) This file is corrupt and cannot be played \n");
...
no_samples:
  {
    ...
    if (!qtdemux->fragmented) {
      ...
      GST_WARNING_OBJECT (qtdemux, "stream has no samples");
                            ...
                            printf("(qtdemux_stbl_init) stream has no samples \n");

      return FALSE;
    } else {

                            printf("(qtdemux_stbl_init) stream has no samples, may pick up samples elsewhere \n");
...
qtdemux_parse_samples (GstQTDemux * qtdemux, QtDemuxStream * stream, guint32 n)
{
  ...
  GST_LOG_OBJECT (qtdemux, "parsing samples for stream fourcc %"
      ...
      stream->pad ? GST_PAD_NAME (stream->pad) : "(NULL)");
                                    ...
                                    printf("(qtdemux_parse_samples) ,stream fourcc %"GST_FOURCC_FORMAT ", pad %s, stbl_idx=%ld, n=%d ,n_spl=%d\n",
                                    GST_FOURCC_ARGS (CUR_STREAM (stream)->fourcc), stream->pad ? GST_PAD_NAME (stream->pad) : "(NULL)", stream->stbl_index, n, stream->n_samples);
  ...
  if (!stream->stsz.data) {
    ...
    g_assert (qtdemux->fragmented);
                                    // printf("g_assert (qtdemux->fragmented); \n");
    goto done;
  }
  ...
  for (i = stream->stsc_index; i < n_samples_per_chunk; i++) {
                      ...
                      // printf("(qtdemux_parse_samples) %s , stream->stsc_chunk_index=%d ,stream->first_chunk=%d, stream->last_chunk=%d\n",  
                      //   stream->pad ? GST_PAD_NAME (stream->pad) : "(NULL)", stream->stsc_chunk_index, stream->first_chunk,stream->last_chunk);
    ...
    if (stream->stsc_chunk_index >= stream->last_chunk
      ...
      GST_LOG_OBJECT (qtdemux,
          ...
          stream->samples_per_chunk, stream->stsd_sample_description_id);
                                    ...
                                    // printf("(qtdemux_parse_samples) goto next chunk ,%s entry %d has first_chunk %d, last_chunk %d, samples_per_chunk %d sample descID:%d \n", 
                                    //  stream->pad ? GST_PAD_NAME (stream->pad) : "(NULL)" ,i, stream->first_chunk, stream->last_chunk, stream->samples_per_chunk, stream->stsd_sample_description_id );
        ...
        GST_DEBUG_OBJECT (qtdemux,
            ...
            j, GST_TIME_ARGS (QTSTREAMTIME_TO_GSTTIME (stream,
                    stream->stco_sample_index)), cur->size);
                        ...
                        printf("(qtdemux_parse_samples) when chunks_are_samples, parse_samples, keyframe sample %d: timestamp %" GST_TIME_FORMAT ",  size %u \n", j, GST_TIME_ARGS (QTSTREAMTIME_TO_GSTTIME (stream,stream->stco_sample_index)), cur->size);
        ...
        for (k = stream->stsc_sample_index; k < samples_per_chunk; k++) {
          GST_LOG_OBJECT (qtdemux, "creating entry %d with offset %"
              ...
              (guint) (cur - samples), chunk_offset, cur->size);
                                        ...
                                        // printf("(qtdemux_parse_samples) pad:%s creating entry %d (sample idx) with offset %" G_GUINT64_FORMAT " and size %d \n", 
                                        //         stream->pad ? GST_PAD_NAME (stream->pad) : "(NULL)", (guint) (cur - samples), chunk_offset, cur->size);
  ...
  if (n + 1 == stream->n_samples) {
    ...
    if (qtdemux->pullbased) {
      GST_DEBUG_OBJECT (qtdemux, "checking for more samples");
                            ...
                            printf("(qtdemux_parse_samples) parsed all available samples; \n");
...
already_parsed:
  {
                              ...
                              printf("(qtdemux_parse_samples) this sample has already been parsed \n");
  ...
  if ((edts = qtdemux_tree_get_child_by_type (trak, FOURCC_edts))) {
    ...
    if (n_segments > 100000 || size < 16 + n_segments * entry_size) {
      ...
      goto done;
                ...
                // printf("(qtdemux_parse_segments) got edit list, at least allocate 1 segment, n_segments is %d", n_segments);
    ...
    for (segment_number = 0; segment_number < n_segments; segment_number++) {
      ...
      GST_DEBUG_OBJECT (qtdemux, "created segment %d time %" GST_TIME_FORMAT
          ...
          stream->timescale);
                                      ...
                                      printf("(qtdemux_parse_segments) created segment %d time %" GST_TIME_FORMAT ", media_start (%" GST_TIME_FORMAT 
                                          "), media_stop (%" GST_TIME_FORMAT "), duration %" GST_TIME_FORMAT ", timescale %u\n",
                                          segment_number, GST_TIME_ARGS (segment->time), GST_TIME_ARGS (segment->media_start), GST_TIME_ARGS (segment->media_stop),
                                          GST_TIME_ARGS (segment->duration), stream->timescale);
  ...
  if (stream->n_segments == 0) {
    ...
    GST_DEBUG_OBJECT (qtdemux, "created dummy segment %" GST_TIME_FORMAT,
        GST_TIME_ARGS (stream_duration));
                                  ...
                                  printf("(qtdemux_parse_segments), no segments, create one dummy segment to play the complete trak %" GST_TIME_FORMAT " \n", GST_TIME_ARGS (stream_duration));
...
{
                                // printf("(qtdemux_parse_traks) each track we associate a new QtDemuxStream \n");
  ...
  GST_LOG_OBJECT (qtdemux, "track language code/id: 0x%04x/%s",
      lang_code, stream->lang_id);
                        ...
                        // printf("(qtdemux_parse_traks) track duration : %" G_GUINT64_FORMAT " , timescale: %" G_GUINT32_FORMAT " \n", stream->duration, stream->timescale);
  ...
  if (qtdemux->fragmented) {
    ...
    qtdemux->moof_offset = 0;
                  ...
                  // printf("QTDEMUX:: in parse_trak, if(qtdemux->fragmented) branch \n");
...
gst_qtdemux_guess_bitrate (GstQTDemux * qtdemux)
{
  ...
  GST_DEBUG_OBJECT (qtdemux, "System bitrate = %" G_GINT64_FORMAT
      ", Stream bitrate = %u", sys_bitrate, bitrate);
                                      ...
                                      printf("(qtdemux_guess_bitrate) System bitrate=%" G_GINT64_FORMAT ", Stream bitrate=%u\n", sys_bitrate, bitrate);
...
qtdemux_prepare_streams (GstQTDemux * qtdemux)
{
                            ...
                            printf("(qtdemux_prepare_streams) num: %u \n",  QTDEMUX_N_STREAMS (qtdemux));
...
qtdemux_expose_streams (GstQTDemux * qtdemux)
{
  ...
  for (i = 0; i < qtdemux->old_streams->len; i++) {
    ...
    if (stream->pad) {
      GstEvent *event;
          printf ("(qtdemux_expose_streams) pushing gst_event_new_eos\n");
      event = gst_event_new_eos ();
...
qtdemux_parse_tree (GstQTDemux * qtdemux)
{
                            ...
                            printf("(qtdemux_parse_tree) we have read the complete moov node now at parse_tree \n");
  ...
  if (version == 1) {
    ...
    return FALSE;
              ...
              // printf("(qtdemux_parse_tree), got mvhd, duration is %" GST_TIME_FORMAT ", timesclae is %" GST_TIME_FORMAT " \n",
              //      GST_TIME_ARGS(QTTIME_TO_GSTTIME (qtdemux, qtdemux->duration)), GST_TIME_ARGS(QTTIME_TO_GSTTIME (qtdemux, qtdemux->timescale)) );
  ...
  if (mvex) {
                ...
                printf("(qtdemux_parse_tree) it is a fragmented file \n");
