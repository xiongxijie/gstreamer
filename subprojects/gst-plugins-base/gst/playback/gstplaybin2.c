...
{
        printf("(gst_play_bin_get_video_tags) \n");
  GstTagList *result;
  GstSourceGroup *group;
    ...
    const gchar * dbg, GstElement ** elem, GstElement * sink)
{
  GST_INFO_OBJECT (playbin, "Setting %s sink to %" GST_PTR_FORMAT, dbg, sink);
                  ...
                  printf("GST-PLAYBIN:: gst_play_bin_set_sink, set %s sink \n", dbg);
...
gst_play_bin_query (GstElement * element, GstQuery * query)
{
        ...
        // printf("GST-PLAYBIN:: gst_play_bin_query %d \n",GST_QUERY_TYPE (query));
...
pad_added_cb (GstElement * decodebin, GstPad * pad, GstSourceGroup * group)
{
  ...
  GST_DEBUG_OBJECT (playbin,
      ...
      GST_DEBUG_PAD_NAME (pad), caps, group);
                ...
                printf("(gstplaybin2/pad_added_cb)Pad name:%s\n", pad_name);


                gchar *caps_str;
                caps_str = gst_caps_to_string(caps);
                printf("(gstplaybin2/pad_added_cb)Caps:%s\n", caps_str);
                g_free(caps_str);
  ...
  if (combine->combiner == NULL && playbin->have_selector) {
    ...
    {
                printf("(gstplaybin2/pad_added_cb) creating input-selector element\n");
    ...
    if ((sinkpad =
      ...
      GST_DEBUG_OBJECT (playbin, "got pad %s:%s from combiner",
          GST_DEBUG_PAD_NAME (sinkpad));
                      ...
                      printf("(gstplaybin2/pad_added_cb) got pad %s:%s from combiner\n",
                      GST_DEBUG_PAD_NAME (sinkpad));
      ...
      if (combine->has_tags) {
                      ...
                      printf("(gstplaybin2/pad_added_cb) combine->has_tags\n");
        ...
        g_object_set_data (G_OBJECT (sinkpad), "playbin.notify_tags_handler",
            ULONG_TO_POINTER (notify_tags_handler));
                ...
                gchar *caps_str_2;
                caps_str_2 = gst_caps_to_string(caps);
                printf("(gstplaybin2/pad_added_cb)g_ptr_array_add sinkpad Caps:%s\n", caps_str_2);
                g_free(caps_str_2);
  ...
  for (i = 0; i < PLAYBIN_STREAM_LAST; i++) {
    ...
    if (combine->sinkpad && combine->srcpad &&
        !gst_pad_is_linked (combine->srcpad)) {
                printf("(gstplaybin2/no_more_pads_cb) sinkpad:%s, srcpad:%s \n", GST_PAD_NAME(combine->sinkpad), GST_PAD_NAME(combine->srcpad));
                // GstCaps *pad_caps = gst_pad_query_caps(combine->srcpad, NULL);
                // gchar *caps_str;
                // caps_str = gst_caps_to_string(pad_caps);
                // printf("(gstplaybin2/no_more_pads_cb) Pad capabilities: %s\n", caps_str);
                // g_free(caps_str);
    ...
    for (i = 0; i < PLAYBIN_STREAM_LAST; i++) {
      ...
      if (combine->srcpad) {
        if (combine->sinkpad == NULL) {
          ...
          gst_object_ref (combine->sinkpad);
                printf("(gstplaybin2/no_more_pads_cb) 222 sinkpad:%s, srcpad:%s \n", GST_PAD_NAME(combine->sinkpad), GST_PAD_NAME(combine->srcpad));

                // GstCaps *pad_caps = gst_pad_query_caps(combine->srcpad, NULL);
                // gchar *caps_str;
                // caps_str = gst_caps_to_string(pad_caps);
                // printf("(gstplaybin2/no_more_pads_cb) 222Pad capabilities: %s\n", caps_str);
                // g_free(caps_str);
...
{
          printf("GST-PLAYBIN:: gst_play_bin_send_event (%s \n",GST_EVENT_TYPE_NAME (event));
...
gst_play_bin_change_state (GstElement * element, GstStateChange transition)
{
      ...
      printf ("(gst_play_bin_change_state) %d", (int)transition);
