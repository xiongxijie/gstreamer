    ...
    gpointer user_data)
{
  ...
  switch (GST_EVENT_TYPE (event)) {
    case GST_EVENT_TAG:
    {
          printf("(uridecodebin/decoded_pad_event_probe) GST_EVENT_TAG\n");
    ...
    GstURIDecodeBin * decoder)
{
  ...
  GST_DEBUG_OBJECT (element, "new decoded pad, name: <%s>", GST_PAD_NAME (pad));
                      ...
                      printf("(uridecodebin/new_decoded_pad_added_cb) new decoded pad, name: <%s>\n",
                      GST_PAD_NAME (pad));
...
gen_source_element (GstURIDecodeBin * decoder)
{
  ...
  if (IS_BLACKLISTED_URI (decoder->uri))
    goto uri_blacklisted;
              ...
              printf("(gsturidecodebin/gen_source_element) register URI handlers!!! \n");
...
make_decoder (GstURIDecodeBin * decoder)
{
  ...
  if (decoder->pending_decodebins) {
    ...
    decodebin = gst_element_factory_make ("decodebin", NULL);
                      ...
                      printf("GST-URI-DECODEBIN:: create the decoder element [decodebin]\n");
...
{
                      printf("GST-URI-DECODEBIN:: type_found \n");
  ...
  if (decoder->is_adaptive) {
    ...
    if (do_download) {
            printf("(uridecodebin/do_download elem_name=downloadbuffer) \n");
      elem_name = "downloadbuffer";
    } else {
            printf("(uridecodebin/do_download elem_name=queue2) \n");
...
setup_streaming (GstURIDecodeBin * decoder)
{
                    ...
                    printf("GST-URI-DECODEBIN:: setup_streaming\n");
