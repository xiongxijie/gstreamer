...
gst_decode_bin_init (GstDecodeBin * decode_bin)
{
                  ...
                  // printf("GST-DECODEBIN:: gst_decode_bin_init \n");
...
printf ("(gstdecodebin2)add static sink pad\n");
...
{
      printf ("(gst_decode_bin_set_sink_caps) Settings force-caps on typefind\n");
  GST_DEBUG_OBJECT (dbin, "Setting new caps: %" GST_PTR_FORMAT, caps);
    ...
    GstCaps * caps, GstDecodeChain * chain, GstDecodeChain ** new_chain)
{
  ...
  GST_DEBUG_OBJECT (dbin, "Pad %s:%s caps:%" GST_PTR_FORMAT,
      GST_DEBUG_PAD_NAME (pad), caps);
              ...
              // const gchar *capname =  gst_structure_get_name(gst_caps_get_structure (caps, 0));
              // printf("GST-DECODEBIN::analyze_new_pad, padname:%s, %s ,caps: %s \n", GST_DEBUG_PAD_NAME (pad), capname);
...
// element here is qtdemux,not decodebin itself
static GList *
    ...
    GstDecodeChain * chain)
{
  ...
  if (dynamic) {
        ...
        // printf ("(connect_element) connect signals on element %s \n",
        // GST_ELEMENT_NAME (element));
    ...
    GstCaps * caps, GstDecodeBin * decode_bin)
{
  ...
  GST_DEBUG_OBJECT (decode_bin, "typefind found caps %" GST_PTR_FORMAT, caps);
              ...
              // if (gst_structure_has_name (gst_caps_get_structure (caps, 0), "video/quicktime")) 
              // {
              //       printf("GST-DECODEBIN::type_found, it is video/quicktime \n");
              // }

              const gchar *capname =  gst_structure_get_name(gst_caps_get_structure (caps, 0));
              printf("GST-DECODEBIN::type_found, it is %s \n", capname);
...
// this callback is actually for such as qtdemux 
// (we only delve into mp4 for now)
static void
pad_added_cb (GstElement * element, GstPad * pad, GstDecodeChain * chain)
{
  ...
  caps = get_pad_caps (pad);
              ...
              // printf ("(decodebin2/pad_added_cb for qtdemux) media_type is %s\n",media_type);
...
pad_removed_cb (GstElement * element, GstPad * pad, GstDecodeChain * chain)
{
  ...
  GST_LOG_OBJECT (pad, "pad removed, chain:%p", chain);
              ...
              printf ("(decodebin2/pad_removed_cb) pad name is %s\n",GST_PAD_NAME (pad));
...
gst_decode_chain_free_internal (GstDecodeChain * chain, gboolean hide)
{
  ...
  if (chain->endpad) {
          ...
          const gchar *pad_type = gst_pad_get_element_private (chain->endpad); 
          printf ("(decodebin/chain_free_internal) pad type is %s\n",pad_type);

    if (chain->endpad->exposed) {
              // GstCaps *pad_caps = gst_pad_query_caps (chain->endpad, NULL);
              // guint caps_sz =  gst_caps_get_size (pad_caps);
              // printf ("(decodebin2) pad to be removed caps size is %d \n", caps_sz);
              // const gchar *pad_name = GST_PAD_NAME(chain->endpad);
              // printf ("444 (decodebin2) gst_element_remove_pad %s\n",
              // pad_name);
    ...
    GstPad * pad)
{
  ...
  GST_DEBUG_OBJECT (dbin, "Creating new chain %p with parent group %p", chain,
      parent);
          // printf ("GST-DECODEBIN::creating new chain %p\n", chain);
  chain->dbin = dbin;
  chain->parent = parent;
  chain->refs = 1;
...
gst_decode_group_new (GstDecodeBin * dbin, GstDecodeChain * parent)
{
  ...
  group->parent = parent;
                ...
                // printf("(gstdecodebin/gst_decode_group_new) make Element multiqueue \n");
  ...
  for (tmp = endpads; tmp; tmp = tmp->next) {
    ...
    if (!dpad->exposed) {
      dpad->exposed = TRUE;
                    ...
                    // GstCaps *pad_caps = gst_pad_query_caps (dpad, NULL);
                    // guint caps_sz =  gst_caps_get_size (pad_caps);
                    // printf ("(decodebin2) pad to be added caps size is %d \n", caps_sz);
                    // gst_caps_unref (pad_caps);
                    // const gchar *pad_name = GST_PAD_NAME(dpad);
                    // printf("333 (gst_decode_bin_expose) add pad:%s\n", pad_name);
...
gst_decode_pad_event (GstPad * pad, GstObject * parent, GstEvent * event)
{
  ...
  if (GST_EVENT_TYPE (event) == GST_EVENT_SEEK && dbin && dbin->decode_chain) {
    GstElement *demuxer = NULL;
                    printf("GST-DECODEBIN:: gst_decode_pad_event %s \n", GST_EVENT_TYPE_NAME (event));
